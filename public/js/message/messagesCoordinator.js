...
function fetchMessages(){
    ...
    // Pass is_ic_message flag to fetch IC messages
...
function showMessages(messages){
    ...
    for(var i=0; i<messages.length; i++){
        ...
        var sender_name = messages[i].sender_name || messages[i].name || 'Unknown User';
...
function displayMessage(msg_id, msg_text, msg_sender_type, msg_timestamp, is_seen, sender_deleted, receiver_deleted, sender_name) {
    ...
    // Create the row div
    // Coordinator sent messages should be 'out', received from client/provider should be 'in'
    ...
    if (msg_sender_type !== 'ISSUE_COORDINATOR') {
        ...
        `;
        senderNameDiv.textContent = sender_name || 'Unknown User';
