...
threads.forEach(function(thread) {
    thread.addEventListener('click', function() {
        ...
        window.location.href = URLROOT + '/Clients/message/' + providerID;
    }); 
});
...
function displayMessage(msg_id, msg_text, msg_sender_type, msg_timestamp, is_seen, sender_deleted, receiver_deleted) {
    ...
    row.className = msg_sender_type === 'CLIENT' ? 'row out' : 'row in';
    row.id = 'msg_' + msg_id;
    
    // If incoming message, add avatar
    if (msg_sender_type !== 'CLIENT') {
    ...
    bubble.className = msg_sender_type === 'CLIENT' ? 'bubble' : 'bubble bubble--out';
    ...
    if (msg_sender_type === 'CLIENT' && is_seen) {
